 ...
 * description :
 */
class HomeMainVoiceNovelAdapter : CustomBaseAdapter<GetListResponse,
        CustomBaseViewHolder>(R.layout.view_home_main_voice_novel_item_layout) {

    override fun onConvert(holder: CustomBaseViewHolder, item: GetListResponse?) {
        val itemRootView = holder.getView<RelativeLayout>(R.id.novel_item_root_view)
        val adRootView = holder.getView<FrameLayout>(R.id.novel_item_ad_layout)
        if (1 != item?.isAd) {
            ...
            GlideUtils.loadImage(
                ...
                item?.album_cover,
                holder.getView(R.id.novel_item_iv_show),
                R.mipmap.home_item_default_icon
            )
            holder.setText(R.id.novel_item_text_name, item?.album_name ?: "")
            holder.setText(R.id.novel_item_voice, item?.anchor_name ?: "")
            holder.setText(R.id.novel_item_create, item?.author_name ?: "")
            holder.getView<CheckBox>(R.id.novel_item_keep_status).isChecked =
